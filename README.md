# nota. — marketing site

Landing page for **nota.**, the iOS app that splits a restaurant bill without an app for your guests: scan the receipt, share a QR code, everyone claims what they had in the browser and pays the host directly (Swish / Vipps / MobilePay).

Static site served via GitHub Pages.

- `index.html` — landing page (EN/SV)
- `privacy.html` — privacy policy (App Store)
- `support.html` — support / FAQ (App Store)
- `assets/screens/` — app screenshots

The guest web app lives separately at [notan.nu](https://notan.nu).
